...
func convertCellView(cv cellView) chart {
	...
	case influxdb.HeatmapViewProperties:
		...
		xAxis := axis{Label: p.XAxisLabel, Prefix: p.XPrefix, Suffix: p.XSuffix, Name: "x", Domain: p.XDomain}
		yAxis := axis{Label: p.YAxisLabel, Prefix: p.YPrefix, Suffix: p.YSuffix, Name: "y", Domain: p.YDomain}

		ch.Axes = []axis{xAxis, yAxis}
		ch.Note = p.Note
